...
\bolumfoto[Celsus Kütüphanesi, antik dünyanın en büyük bilgi kaynaklarından biriydi. Bu ek de RISC-V buyruk kümesinin başvuru kaynağıdır. Yazarken, okurken ve hata ayıklarken her an dönüp bakılacak bir referanstır.]{gorseller/ekC-efes.jpg}{Efes Antik Kenti, Celsus Kütüphanesi -- İzmir}
...
RISC-V, yazılım katmanlarını birbirinden yalıtmak için üç ayrıcalık düzeyi tanımlar (Tablo~\ref{tab:ekc-ayricalik}):\index{ayrıcalık düzeyleri}\index{RISC-V!ayrıcalık düzeyleri}
...
Yükleme buyruklarında (\texttt{lw}) veri bellekten yazmaca \emph{okunur}. Hedef bir yazmaçtır. Saklama buyruklarında (\texttt{sw}) ise veri yazmaçtan belleğe \emph{yazılır}. İlk işlenen bellekten değil, belleğe \emph{gönderilecek} olan yazmacı gösterir.
\end{bilginot}

\subsection{Buyruk Kategorileri}

RISC-V buyruklarını beş ana kategoride toplamak mümkündür (Tablo~\ref{tab:ekc-buyruk-kategorileri}; ayrıntılı açıklama Bölüm~\ref{chap:buyruk-kumesi}'te verilmektedir):
...
Bir buyruğun ikili kodlamasını somut bir örnek üzerinden inceleyelim. \texttt{add x5, x6, x7} buyruğu R türündedir (bkz.\ Şekil~\ref{fig:ekc-r-tipi}) ve aşağıdaki gibi kodlanır:
...
Şekil~\ref{fig:ekc-yigit-cerceve}, bir alt yordam çağrısı sırasında yığıt çerçevesinin yapısını göstermektedir:
...
% --- R Türü ---
\textbf{R türü} (Şekil~\ref{fig:ekc-r-tipi}): İki kaynak ve bir hedef yazmaç içerir; işlev kodu alanları (funct7, funct3) yapılacak işlemi belirler. Aritmetik ve mantık buyrukları bu biçimi kullanır.
...
% --- I Türü ---
\textbf{I türü} (Şekil~\ref{fig:ekc-i-tipi}): 12 bitlik işaretli anlık değer, bir kaynak yazmaç ve bir hedef yazmaç içerir. Anlık aritmetik, yükleme ve bazı sistem buyrukları bu biçimi kullanır.
...
% --- S Türü ---
\textbf{S türü} (Şekil~\ref{fig:ekc-s-tipi}): Anlık değer iki ayrı alana (imm[11:5] ve imm[4:0]) bölünmüştür; iki kaynak yazmaç içerir, hedef yazmaç alanı yoktur. Saklama buyrukları (\texttt{sw}, \texttt{sh}, \texttt{sb}) bu biçimi kullanır.
...
% --- B Türü ---
\textbf{B türü} (Şekil~\ref{fig:ekc-b-tipi}): S türüne benzer, ancak anlık değer bit sıralaması farklıdır; iki kaynak yazmaç içerir, hedef yazmaç yoktur. Dal ofseti çift sayıdır (en düşük bit her zaman 0). Dallanma buyrukları (\texttt{beq}, \texttt{bne} vb.) bu biçimi kullanır.
...
% --- U Türü ---
\textbf{U türü} (Şekil~\ref{fig:ekc-u-tipi}): 20 bitlik anlık değer ve bir hedef yazmaç içerir; bu değer yazmacın üst 20 bitine yerleştirilir. \texttt{lui} ve \texttt{auipc} buyrukları bu biçimi kullanır.
...
% --- J Türü ---
\textbf{J türü} (Şekil~\ref{fig:ekc-j-tipi}): U türüne benzer alan genişliğinde; 20 bitlik anlık değer farklı bit sıralamasıyla kodlanır. \texttt{jal} buyruğu bu biçimi kullanarak 20 bitlik ofsetle koşulsuz atlama gerçekleştirir.
...
RISC-V çağrı kurallarına göre argümanlar \texttt{a0}--\texttt{a7} yazmaçlarıyla aktarılır; dönüş değeri \texttt{a0}'a yazılır. \texttt{toplam} bir \emph{yaprak işlevdir}. Başka bir işlev çağırmadığı ve yalnızca argüman yazmaçlarını kullandığı için yığıt çerçevesine gerek duymaz; \texttt{ra} olduğu gibi korunur. Buna karşılık başka bir işlev çağıran ya da kaydedilen yazmaçları (\texttt{s0}--\texttt{s11}) kullanan işlevler, kullandıkları kaydedilen yazmaçları ve \texttt{ra}'yı yığıta saklayıp geri yüklemek zorundadır. \texttt{ret} sözde buyruğu, çevirici tarafından \texttt{jalr zero, 0(ra)} olarak açılır.
\end{ornek}
