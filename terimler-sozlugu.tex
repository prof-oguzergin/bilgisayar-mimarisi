...
% Tekrarlanan başlık ve "devamı sonraki sayfada" satırı sayfa kavramına
% bağlıdır. EPUB modunda boş bırakılır; tablo hizalaması içinde \ifdefined
% dallanması yapılamadığından makro dışarıda tanımlanır.
\ifdefined\HCode
\def\ltdevam{}
\else
\def\ltdevam{%
...
en anlamlı bit (\texttt{MSB}) & Most Significant Bit (\texttt{MSB}) \\
en anlamsız bit (\texttt{LSB}) & Least Significant Bit (\texttt{LSB}) \\
...
eşzamanlı çoklu iş parçacığı (\texttt{SMT}) & Simultaneous Multithreading (\texttt{SMT}) \\
...
ödünleşme & trade-off \\
Dışlayan VEYA kapısı & \texttt{XOR} gate \\
...
sırasız yürütüm & out-of-order execution \\
...
% Tekrarlanan başlık/alt satır: sayfa kavramına bağlı, EPUB'da boş (bkz. üstteki tablo).
\ifdefined\HCode
\def\ltdevam{}
\else
\def\ltdevam{%
...
Least Significant Bit (\texttt{LSB}) & en anlamsız bit (\texttt{LSB}) \\
linker & bağlayıcı \\
...
Most Significant Bit (\texttt{MSB}) & en anlamlı bit (\texttt{MSB}) \\
...
Simultaneous Multithreading (\texttt{SMT}) & eşzamanlı çoklu iş parçacığı (\texttt{SMT}) \\
...
%% --- X ---
\texttt{XOR} gate & Dışlayan VEYA kapısı \\
