...
\begin{itemize}[nosep, leftmargin=*]
    ...
    \item Proje çıktılarını belirlenen değerlendirme ölçütlerine göre (bkz.\ Tablo~\ref{tab:ekd-degerlendirme}) sınamak.
\end{itemize}
\end{tcolorbox}
...
Verilog'da temel tasarım birimi \emph{modül}dür. Her modülün giriş/çıkış kapıları ve iç mantığı vardır.
...
\begin{itemize}[leftmargin=2em]
    \item \texttt{wire}\index{Verilog!wire}: Birleşik mantıkta kullanılır. Fiziksel bir teli temsil eder; \texttt{assign} ile sürekli atama yapılır.
...
Doğrulama ortamı\index{doğrulama ortamı}\index{Verilog!doğrulama ortamı} (\emph{testbench}), tasarlanan modülü doğrulamak için yazılan benzetim ortamıdır. Doğrulama ortamı modülünün giriş/çıkış kapısı yoktur; test edilen modülü içsel olarak örnekler (instantiate).
...
Bölüm~\ref{chap:islemci-tasarimi}'da öğrenilen yazmaç öbeğinin\index{yazmaç öbeği} Verilog ile gerçeklenişi, işlemci projeleri için temel bir yapı taşıdır. Aşağıdaki modül, RISC-V'in 32 adet 32 bitlik yazmaç öbeğini tanımlar. İki okuma kapısı ve bir yazma kapısı içerir; \texttt{x0} yazmacı her zaman sıfırdır.
...
Bu modül, Bölüm~\ref{chap:islemci-tasarimi}'daki tek çevrimli veri yolu şemasının doğrudan Verilog karşılığıdır. Okuma kapıları bileşimsel (\texttt{assign}), yazma kapısı ise saat kenarına bağlıdır. Şekil~\ref{fig:ekd-yazmac-blok}'da bu modülün blok diyagramı gösterilmektedir.
...
\begin{tikzpicture}[every node/.style={font=\footnotesize}]
  ...
  % Sol: giriş kapıları (kutunun dışında, yazılar okların üstünde)
  ...
  % Sağ: çıkış kapıları (kutunun dışından başlıyor)
...
\end{tikzpicture}
\caption[Yazmaç öbeği blok diyagramı]{Yazmaç öbeği blok diyagramı. İki okuma kapısı bileşimsel (saat bağımsız), yazma kapısı saat kenarına bağlıdır. \texttt{x0} yazmacı donanım tarafından sıfıra sabitlenmiştir.}
...
Bu bölümde, kitabın farklı konularını pekiştiren üç proje sunulmaktadır; projelerin genel özeti Tablo~\ref{tab:ekd-projeler}'de verilmiştir. Projeler artan karmaşıklık düzeyinde tasarlanmıştır; her biri bir dönem ödevi veya laboratuvar çalışması olarak kullanılabilir.
...
\textbf{Amaç:} Bölüm~\ref{chap:boruhatti}'deki beş aşamalı boru hattını (IF--ID--EX--MEM--WB) yazılımla benzetim yaparak veri ve denetim sorunlarını gözlemlemek.\smallskip
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item Aşağıdaki sorun çözüm tekniklerini gerçekleyin:
    \begin{itemize}[leftmargin=1.5em]
        \item \textbf{Veri sorunu:} İletme (forwarding) birimi ve gerektiğinde durdurma (stall) mantığı.
        \item \textbf{Denetim sorunu:} Dallanmada basit ``her zaman dallanma yok'' öngörüsü; yanlış öngörüde boru hattını boşaltma (flush).
    \end{itemize}
    ...
    \begin{itemize}[leftmargin=1.5em]
        ...
        \item Sorun türlerine göre dağılım
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item Parametre taraması yaparak sonuçları tablolaştırın; örnek bir tarama çıktısı Tablo~\ref{tab:ekd-onbellek-tarama}'de gösterilmiştir.
\end{enumerate}
...
\begin{enumerate}[(a)]
\item Üçüncü bir okuma kapısı (\texttt{oku\_adr3}, \texttt{oku\_veri3}) ekleyin.
...
\begin{alistirma}[\zor]
Listeleme~\ref{lst:ekd-yazmac}'daki yazmaç öbeği modülüne \emph{yazma yönlendirmesi} (write forwarding) ekleyin. Aynı çevrimde yazılan ve okunan yazmaç aynıysa, okuma kapısında yeni yazılan değer görünsün. Bu özelliğin boru hattındaki veri sorunlarını azaltmaya nasıl katkı sağladığını açıklayın.
\end{alistirma}
